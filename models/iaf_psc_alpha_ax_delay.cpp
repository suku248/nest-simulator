...
nest::RecordablesMap< nest::iaf_psc_alpha_ax_delay > nest::iaf_psc_alpha_ax_delay::recordablesMap_;
...
template <>
void
RecordablesMap< iaf_psc_alpha_ax_delay >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &iaf_psc_alpha_ax_delay::get_V_m_ );
  insert_( names::I_syn_ex, &iaf_psc_alpha_ax_delay::get_I_syn_ex_ );
  insert_( names::I_syn_in, &iaf_psc_alpha_ax_delay::get_I_syn_in_ );
}
...
iaf_psc_alpha_ax_delay::Parameters_::Parameters_()
  : Tau_( 10.0 )             // ms
  , C_( 250.0 )              // pF
  , TauR_( 2.0 )             // ms
  , E_L_( -70.0 )            // mV
...
iaf_psc_alpha_ax_delay::State_::State_()
  : y0_( 0.0 )
  , dI_ex_( 0.0 )
  , I_ex_( 0.0 )
  , dI_in_( 0.0 )
...
void
iaf_psc_alpha_ax_delay::Parameters_::get( DictionaryDatum& d ) const
{
...
double
iaf_psc_alpha_ax_delay::Parameters_::set( const DictionaryDatum& d, Node* node )
{
...
void
iaf_psc_alpha_ax_delay::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
  def< double >( d, names::V_m, y3_ + p.E_L_ ); // Membrane potential
}

void
iaf_psc_alpha_ax_delay::State_::set( const DictionaryDatum& d, const Parameters_& p, double delta_EL, Node* node )
{
...
iaf_psc_alpha_ax_delay::Buffers_::Buffers_( iaf_psc_alpha_ax_delay& n )
  : logger_( n )
{
}

iaf_psc_alpha_ax_delay::Buffers_::Buffers_( const Buffers_&, iaf_psc_alpha_ax_delay& n )
  : logger_( n )
{
}
...
iaf_psc_alpha_ax_delay::iaf_psc_alpha_ax_delay()
  : ArchivingNode()
  , P_()
  , S_()
  , B_( *this )
...
iaf_psc_alpha_ax_delay::iaf_psc_alpha_ax_delay( const iaf_psc_alpha_ax_delay& n )
  : ArchivingNode( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
iaf_psc_alpha_ax_delay::init_buffers_()
{
...
void
iaf_psc_alpha_ax_delay::calibrate()
{
  ...
  // a double in ms. The grid based iaf_psc_alpha_ax_delay can only handle refractory
  ...
  //
  // The definition of the refractory period of the iaf_psc_alpha_ax_delay is consistent
...
void
iaf_psc_alpha_ax_delay::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
