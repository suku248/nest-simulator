...
#ifndef IAF_PSC_ALPHA_AX_DELAY_H
#define IAF_PSC_ALPHA_AX_DELAY_H
...
``iaf_psc_alpha_ax_delay`` is an implementation of a leaky integrate-and-fire model
...
The ``iaf_psc_alpha_ax_delay`` is the standard model used to check the consistency
...
.. note::
   ...
   optimization levels. A future version of iaf_psc_alpha_ax_delay will probably
...
class iaf_psc_alpha_ax_delay : public ArchivingNode
{

public:
  iaf_psc_alpha_ax_delay();
...
private:
  ...
  struct Buffers_
  {
    ...
    Buffers_( iaf_psc_alpha_ax_delay& );
    Buffers_( const Buffers_&, iaf_psc_alpha_ax_delay& );
    ...
    UniversalDataLogger< iaf_psc_alpha_ax_delay > logger_;
  };
  ...
  /**
   * @defgroup iaf_psc_alpha_ax_delay_data
  ...
  static RecordablesMap< iaf_psc_alpha_ax_delay > recordablesMap_;
};
...
inline port
iaf_psc_alpha_ax_delay::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
iaf_psc_alpha_ax_delay::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
iaf_psc_alpha_ax_delay::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
iaf_psc_alpha_ax_delay::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
iaf_psc_alpha_ax_delay::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef iaf_psc_alpha_ax_delay_H */
